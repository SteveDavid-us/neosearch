...
(function () {
    ...
    var Swiper = function (container, params) {
        ...
        function handleKeyboard(e) {
            ...
            // Directions locks
            if (!s.params.allowSwipeToNext && (isH() && kc === s.params.nextKey || !isH() && kc === 40)) {
                return false;
            }
            if (!s.params.allowSwipeToPrev && (isH() && kc === s.params.prevKey || !isH() && kc === 38)) {
                return false;
            }
            if (e.shiftKey || e.altKey || e.ctrlKey || e.metaKey) {
                return;
            }
            if (document.activeElement && document.activeElement.nodeName && ($(document.activeElement).is('input, select, textarea, div.modal.in') || $(document.activeElement).parents('div.modal.in').length) && !$(document.activeElement).is('#clipboard')) {
                return;
            }
            if (kc === s.params.prevKey || kc === s.params.nextKey /*|| kc === 38 || kc === 40 */) {
                var inView = false;
            ...
            if (isH()) {
                if (kc === s.params.nextKey || kc === s.params.prevKey) {
                    if (e.preventDefault) e.preventDefault();
                    else e.returnValue = false;
                }
                if (kc === s.params.nextKey) s.slideNext();
                if (kc === s.params.prevKey) s.slidePrev();
            }
            else {
                if (kc === 38 || kc === 40) {
                    if (e.preventDefault) e.preventDefault();
